...
// UPLOAD IMAGE CAR MAINTENANCE
const validateImageCreateMaintenance = async (req, res, next) => {
    let data = { ...Constants.ResultData };
...
const uploadImageMaintenanceToFirebase = async (req, res, next) => {
    if (req.file) {
        let data = { ...Constants.ResultData };
        const currentDate = Date.now();
        const randomNumber = Math.floor(Math.random() * 10);
        ...
        const storageRef = ref(
            storage,
            `imagesCarMaintenance/${currentDate}_${randomNumber}_${req.file.originalname}`
        );
        const metadata = {
            contentType: req.file.mimetype,
        };
